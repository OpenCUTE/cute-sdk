...
static uint32_t zero_bias[TEST_SEQ_LEN][TEST_SEQ_LEN] CUTE_TEST_ALIGN;
...
static inline void init_inputs(void)
{
    ...
    for (int row = 0; row < TEST_N_HEAD_KV * TEST_VALUE_DIM; row++) {
        for (int col = 0; col < TEST_EMBED_DIM; col++) {
            proj_v_weight[row][col] =
                (int8_t)(((row * 3 + col * 5) % 7) - 3);
        }
    }
    init_causal_mask();
}
