#include "../fusion_matmul_post_utils.h"

static float residual[CUTE_FUSION_OUTPUT_M][CUTE_FUSION_OUTPUT_N] CUTE_TEST_ALIGN;
static float output[CUTE_FUSION_OUTPUT_M][CUTE_FUSION_OUTPUT_N] CUTE_TEST_ALIGN;

int main(void)
{
    cute_resadd_ctx_t ctx = {
        .residual = &residual[0][0],
        .residual_stride = CUTE_FUSION_OUTPUT_N * sizeof(residual[0][0]),
    };

    cute_fusion_init_scales();
    cute_fusion_init_residual(residual);
    cute_fusion_run_pipeline(output,
                             CUTE_FUSION_OUTPUT_N * sizeof(output[0][0]),
                             sizeof(output[0][0]),
                             cute_post_dequant_resadd,
                             &ctx);
    cute_test_host_verify_barrier();
    return 0;
}
