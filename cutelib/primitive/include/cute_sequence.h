                                       ...
                                       int rows, int cols)
{
    ...
    for (int r = 0; r < rows; r++) {
        ...
        for (int k = 0, avl = half_dim; avl > 0; k += vl, avl -= vl) {
            ...
            vuint16m2_t real_bf16 = cute_f32m4_to_bf16m2_trunc(real_out, vl);
            vuint16m2_t imag_bf16 = cute_f32m4_to_bf16m2_trunc(imag_out, vl);
            __riscv_vse16_v_u16m2(output_row + k, real_bf16, vl);
            __riscv_vse16_v_u16m2(output_row + half_dim + k, imag_bf16, vl);
        }
    }
}
                                                 ...
                                                 int rows, int cols)
{
    for (int r = 0; r < rows; r++) {
        ...
        for (int c = 0, avl = cols; avl > 0; c += vl, avl -= vl) {
            ...
            vfloat32m4_t vec = __riscv_vle32_v_f32m4(&scratch[c], vl);
            vfloat32m4_t normalized = __riscv_vfmul_vv_f32m4(vec, inv_sum_exp_vec, vl);
            vuint16m2_t normalized_bf16 = cute_f32m4_to_bf16m2_trunc(normalized, vl);
            __riscv_vse16_v_u16m2(&output_row[c], normalized_bf16, vl);
        }
    }
}
